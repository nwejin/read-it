...
async function fetchStats(userId: string): Promise<StatsData> {
  ...
  const { data } = await supabase
    .from('user_books')
    .select('is_owned, read_status, rating, read_at')
    .eq('user_id', userId);
  const rows = data ?? [];
  ...
  rows
    .filter((r) => r.read_status === 'read' && r.read_at)
    .forEach((r) => {
      const year = r.read_at.slice(0, 4);
      const month = parseInt(r.read_at.slice(5, 7)) - 1;
...
export default function LibraryView({ userId, isOwner, nickname }: LibraryViewProps) {
  ...
  const filteredItems = useMemo(() => {
    ...
    return sortedItems.filter(
      (item) => item.book.title.toLowerCase().includes(q) || (item.book.author ?? '').toLowerCase().includes(q)
    );
  ...
  return (
    <div className="max-w-lg mx-auto">
      {/* 헤더 */}
      <div className="sticky top-0 bg-white z-sticky px-5 pt-14 pb-0 border-b border-[#F0F0F0]">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            {!isOwner && (
              <button onClick={() => router.back()} className="text-[#111] -ml-1 p-1">
              ...
              {TABS.map((tab) => (
                <button
                  ...
                  className={`shrink-0 pb-3 text-base font-medium border-b-2 transition-colors ${
                    activeTab === tab.key ? 'text-[#111] border-[#111]' : 'text-[#bbb] border-transparent'
                  }`}>
                  {tab.label}
                </button>
              ))}
      ...
      {showStats && stats && (
        <div className="px-5 pt-6 pb-10 space-y-6">
          ...
          {/* 월별 독서 차트 */}
          {(() => {
            const years = Object.keys(stats.monthlyByYear).sort().reverse();
            const currentData = stats.monthlyByYear[selectedYear] ?? Array(12).fill(0);
            const maxCount = Math.max(...currentData, 1);
            const BAR_MAX_H = 80;
            return (
              <div>
                <div className="flex items-center justify-between mb-4">
                  <p className="text-sm font-semibold text-[#888] uppercase tracking-widest">월별 완독</p>
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="text-[#bbb] disabled:opacity-30 p-0.5">
                      <ChevronLeft className="w-4 h-4" strokeWidth={2} />
                    </button>
                    <span className="text-sm font-semibold text-[#111] w-10 text-center">{selectedYear}</span>
                    <button
                      onClick={() => {
                        const i = years.indexOf(selectedYear);
                ...
                {years.length === 0 ? (
                  <p className="text-sm text-[#bbb] text-center py-6">완독 날짜를 기록한 책이 없어요</p>
                ) : (
                  <div className="flex items-end gap-1">
                    {currentData.map((count, i) => (
                      <div key={i} className="flex-1 flex flex-col items-center gap-1">
                        <span className="text-xs text-[#111] font-medium h-4 leading-none">
                          {count > 0 ? count : ''}
                        </span>
                        <div
                          className="w-full bg-[#111] rounded-sm transition-all duration-500"
                          style={{
                            height: `${Math.round((count / maxCount) * BAR_MAX_H)}px`,
                            minHeight: count > 0 ? '4px' : '0px',
                          }}
                        />
                        <span className="text-xs text-[#888] font-medium">{i + 1}</span>
                      </div>
                    ))}
                  </div>
                )}
      ...
      {/* 목록 */}
      {!showStats && (
        <div ref={listRef} className="px-5 pt-1" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
          {isFetching && (
              ...
              {filteredItems.map((item) => (
                <button
                  ...
                  <div className="relative w-12 h-17 shrink-0 bg-[#F0F0F0] rounded-md overflow-hidden">
                    {item.book.cover ? (
                      <Image src={item.book.cover} alt={item.book.title} fill sizes="48px" className="object-cover" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center text-[#ccc] text-[10px]">없음</div>
                    )}
                  </div>
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-center">
                    ...
                    <div className="flex gap-1.5 mt-2 flex-wrap">
                      {item.userBook.is_owned ? (
                        ...
                        <span className="px-2.5 py-0.5 bg-[#F0F0F0] text-[#888] text-sm font-medium rounded-full">
                          미보유
                        </span>
                      )}
                      ...
                      {item.userBook.read_status === 'read' && (
                        <span className="px-2.5 py-0.5 bg-[#F0F0F0] text-sm font-medium rounded-full">
                          <span className="text-amber-400">
                            {'★'.repeat(item.userBook.rating ?? 0)}
                          </span>
                          <span className="text-[#E0E0E0]">
                            {'☆'.repeat(5 - (item.userBook.rating ?? 0))}
                          </span>
                          {item.userBook.read_at && (
                            <span className="text-[#bbb] ml-1">
                              · {item.userBook.read_at.slice(0, 7).replace('-', '.')}
                            </span>
                          )}
                        </span>
                      )}
